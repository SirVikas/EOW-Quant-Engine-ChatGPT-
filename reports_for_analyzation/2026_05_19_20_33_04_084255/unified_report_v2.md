...
|---|---|
| System State | ACTIVE |
| Trading Activity | RECENT (15 min ago) |
| Profitability | LOSS (PF=0.33, net=$-64.09) |
| Key Problem | Avg loss ($-0.58) is 0.7× avg win ($0.85); fees $30.15 consume 32.0% of gross |
...
| Metric | Value |
|---|---|
| Signals Generated (window) | 163 |
| Signals Passed → Traded | 20 |
| Signals Rejected | 0 |
| Pass Rate | 12.3% |
| Reject Rate | 0.0% |
| Rejection Rate (window %) | 0.0% |
| Mins Since Last Trade | 15.3 |
| Signals / hour | 163.0 |
| Trades / hour | 20.00 |
| Execution Gap | 143 signal(s) → 0 trades |
| Dominant Block | RL_TOXIC (43 rejection(s)) |
...
**WHY:**
- Profit factor 0.33 < 1.0 — system in drawdown recovery posture
- Win rate 18.2% below 45% — signal quality degraded
- Trade Activator TIER_2 — filters relaxed (score_min=0.400)
...
| Metric | Value |
|---|---|
| Current Equity | $935.90 |
| Net PnL | $-64.09 |
| Fees Paid | $30.15 |
| Capital Deployed | 3.4% |
| Capital Idle | 96.6% |
| Daily Risk Used | $250.35 |
...
- - MEAN_REVERTING@MeanReversion_PAPER_SPEED: edge=-0.342 wr=16% n=50 [DISABLED]
...
**RL_TOXIC BLOCK (43 rejection(s))**
  Cause: RL_TOXIC gating all signals in current window
  Impact: 43 trade opportunity(s) missed
  Fix: Review RL_TOXIC conditions and threshold settings

**LOW PROFIT FACTOR (0.33)**
  Cause: Avg loss (-0.58) is 0.7× avg win (0.85)
...
**HIGH FEE DRAG (32% of gross)**
  ...
  Impact: Fees consuming $30.15 of gross turnover
  Fix: Increase MIN_NOTIONAL_USDT; reduce trade frequency

**RECURRING ERROR: WS_001 (18 times)**
...
**PRIMARY CAUSE:**
**Negative expectancy (PF=0.33)** — avg loss 0.7× avg win. Signal pipeline functioning; structural problem is insufficient reward-to-risk.

**SECONDARY CAUSES:**
- Avg loss (-0.58) > avg win (0.85) across 203 trades
...
Trades (total):  203  |  PF: 0.329  |  WR: 18.2%
Gate:            can_trade=True  reason=ALL_CLEAR
Tier:            TIER_2  score_min=0.4  vol_mult=0.3×
Idle:            15.3 min
Signals/hr:      163.0  Skips(window):   0
Top Errors:
  WS_001:gap=31.6s: 2×
  WS_001:gap=30.8s: 1×
  WS_001:gap=32.9s: 1×
  WS_002:gap=61.1s attempt=2: 1×
  WS_001:gap=45.7s: 1×
```
...
| Allowed / Blocked | 33 / 43 |
| Allow Rate | 43% |
| Explore Trades | 0 (0%) |
| Exploit Trades | 33 (100%) |
| Confidence ×1.25 Fires | 0 |
| Score Floor −0.05 Fires | 0 |
| Score Floor +0.05 Fires | 66 |
...
|---|---|---|---|---|---|
| MEAN_REVERTING|LONDON|MeanReversion | -0.4216 | -0.2092 | 21% | 216 | ⚠️ YES |
| MEAN_REVERTING|ASIA|MeanReversion | -0.3053 | -0.0770 | 18% | 187 | No |
| MEAN_REVERTING|NY|MeanReversion | -0.2351 | -0.0465 | 17% | 274 | No |