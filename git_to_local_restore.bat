...
"%GIT_SHELL%" -c "cd '%TARGET_DIR%'; if [ -d '.git' ]; then git rm --cached data/eow_lake.db 2>/dev/null; git rm --cached -r data/exports/ 2>/dev/null; git stash; git pull origin main; git stash drop 2>/dev/null; else git clone '%REPO_URL%' .; fi"
